#!/bin/bash
# Test string filter glob patterns
...
$TV -c 'load ./tmp/strings.csv | filter name==*apple*' 2>/dev/null | grep -q '(2 rows)' && echo "  name==*apple*: PASS" || exit 1

# ends with: 2 rows (cherry, blueberry)
$TV -c 'load ./tmp/strings.csv | filter name==*rry' 2>/dev/null | grep -q '(2 rows)' && echo "  name==*rry: PASS" || exit 1

# starts with: 2 rows (banana, blueberry)
$TV -c 'load ./tmp/strings.csv | filter name==b*' 2>/dev/null | grep -q '(2 rows)' && echo "  name==b*: PASS" || exit 1
